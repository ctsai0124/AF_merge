...
def ocr_available():
    """檢查 OCR 相關套件與繁中語言包是否可用"""
    try:
        import cv2, pytesseract  # noqa
...
def _ocr_cells(img_gray):
    """用表格線切出儲存格，逐格辨識數字。回傳 [(列, [字串,...])]"""
    import cv2
    import pytesseract
    ...
    cnts, _ = cv2.findContours(cv2.bitwise_not(grid), cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    cells = []
    for c in cnts:
        x, y, cw, ch = cv2.boundingRect(c)
        if 25 < cw < w * 0.15 and 15 < ch < 80:
    ...
    for row in rows:
        ...
        for (x, y, cw, ch) in row:
            pad = 3
            cell = img_gray[max(0, y + pad):y + ch - pad, max(0, x + pad):x + cw - pad]
            if cell.size == 0:
                vals.append('')
                continue
            cell = cv2.resize(cell, None, fx=2.5, fy=2.5, interpolation=cv2.INTER_CUBIC)
            cell = cv2.threshold(cell, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
            vals.append(pytesseract.image_to_string(cell, lang='eng', config=cfg).strip())
        out.append(vals)
    return out


def ocr_pdf_numbers(file_bytes, max_pages=3):
    """
    對掃描 PDF 做逐格數字辨識。
    回傳 [[字串,...], ...]（每列一組），失敗或未安裝套件時回傳 []
    """
        ...
        try:
            ...
            with pdfplumber.open(tmp.name) as pdf:
                for page in pdf.pages[:max_pages]:
                    pil = page.to_image(resolution=300).original.convert('L')
                    arr = np.array(pil)
                    rows.extend(_ocr_cells(arr))
            return rows
        finally:
            os.unlink(tmp.name)
    except Exception:
...
def fix_name(raw, known_names):
    """用已知名單修正 OCR 的姓名，回傳 (修正後姓名, 是否有改動)"""
    raw = (raw or '').strip()
...
def build_manual_rows(af_records, ocr_rows=None):
    """
    產生手動輸入表格的初始資料：以 AF 人員為基準，
    有 OCR 結果就依金額嘗試預填。
    """
    prefill = {}
    if ocr_rows:
        # 以「薪俸金額」當索引，嘗試對回 AF 人員
        by_pay = {}
        for a in af_records:
            by_pay.setdefault(a['薪俸'], []).append(a)
        for vals in ocr_rows:
            nums = []
            for v in vals:
                try:
                    nums.append(int(v))
                    ...
                    nums.append(None)
            for n in nums:
                if n and n in by_pay and len(by_pay[n]) == 1:
                    tgt = by_pay[n][0]['身分證']
                    prefill.setdefault(tgt, {})['薪俸'] = n
                    break

    rows = []
    for a in af_records:
        pf = prefill.get(a['身分證'], {})
        rows.append({
            '身分證': a['身分證'], '姓名': a['姓名'], '單位': a.get('單位', ''),
            '薪俸': pf.get('薪俸', ''), '專業加給': pf.get('專業加給', ''),
            '主管加給': pf.get('主管加給', ''), '導師特教': pf.get('導師特教', ''),
            '應發金額': pf.get('應發金額', ''),
            'af': {f: a.get(f, 0) for f in FIELDS},
        })
    return rows


def compare_manual(manual_rows, af_records):
    """以手動輸入（或 OCR 校正後）的資料與 AF 比對"""
    af_by_id = {a['身分證']: a for a in af_records}
    results = []

    for r in manual_rows:
        a = af_by_id.get(r.get('身分證'))
        if not a:
            continue
        # 全部留白 → 視為清冊無此人
        if all(str(r.get(f, '')).strip() == '' for f in FIELDS):
            results.append({'姓名': a['姓名'], '職稱': '', '單位': a.get('單位', ''),
                            '身分證': a['身分證'], '配對依據': '手動',
                            '狀態': 'af_only', '差異': []})
            continue

        p = {f: _n(r.get(f, 0)) for f in FIELDS}
        p['應發金額'] = _n(r.get('應發金額', 0))
        ok, adiff = arith_check(p)

        diffs = []
        for f in FIELDS:
            if p[f] != a.get(f, 0):
                diffs.append({'欄位': FIELD_LABEL[f], '清冊': p[f],
                              'AF': a.get(f, 0), '差額': a.get(f, 0) - p[f]})

        results.append({
            '姓名': a['姓名'], '職稱': '', '單位': a.get('單位', ''),
            '身分證': a['身分證'], '配對依據': '手動輸入',
            '狀態': 'diff' if diffs else 'ok', '差異': diffs,
            '_arith_ok': ok, '_arith_diff': adiff,
        })

    order = {'diff': 0, 'af_only': 2, 'ok': 3}
    results.sort(key=lambda r: (order.get(r['狀態'], 1), r['姓名']))
    summary = {
        '一致': sum(1 for r in results if r['狀態'] == 'ok'),
        '有差異': sum(1 for r in results if r['狀態'] == 'diff'),
        '清冊有AF無': 0,
        'AF有清冊無': sum(1 for r in results if r['狀態'] == 'af_only'),
        '待人工指定': 0,
        '加總不符': sum(1 for r in results if r.get('_arith_ok') is False),
    }
    return {'summary': summary, 'results': results, 'ambiguous': []}

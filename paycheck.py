...
# ── 人員類別（細分類 → 官方五類）─────────────────────────
# 官方五類名稱與依據來源於 2026/9/9 修正：Leo 提供的官方「薪資互核結果表
# 範本」（機關每月要產製、送交核對的正式報表）裡「各類人員類別」欄的
# 填寫說明白紙黑字寫明五類為：一般人員、教育警察人員、政務人員、職工、
# 約聘僱人員——這才是真正的官方依據，之前用 static/稽核表-.docx「六、
# 各類表別」猜的「教育人員/職員/技工工友/聘用人員/約僱人員」不準確
# （字面用語有出入，且聘用/約僱在官方報表裡合併算同一類，不是分開兩類），
# 已訂正。對照薪俸表別代碼：
#   A0001   公教人員俸表                    → 一般人員
#   A00011  公教人員俸表(教育警察人員)       → 教育警察人員
#   A0003   技工工友工餉表                  → 職工
#   A0004   聘用人員薪點表                  → 約聘僱人員（跟A0005合併算一類）
#   A0005   約僱人員薪點表                  → 約聘僱人員（跟A0004合併算一類）
#   （政務人員）                            → 目前兩份真實 AF 範例裡都沒出現
#             對應代碼，Leo 還在確認中；先保留這個官方類別的位置（會出現在
#             統計表、永遠是0人），SALARY_TABLE_CATEGORY 故意不對應任何代碼，
#             等代碼確定後只要在這個字典補一行即可，不影響其他四類運作。
OFFICIAL_CATEGORIES = ['一般人員', '教育警察人員', '政務人員', '職工', '約聘僱人員']

SALARY_TABLE_CATEGORY = {
    'A00011': '教育警察人員',
    'A0001': '一般人員',
    'A0003': '職工',
    'A0004': '約聘僱人員',
...
def category_from_salary_table(code):
    """依 AF「薪俸表別」代碼推導官方五類；辨識不出時回傳 None。
    這是舊格式清冊（沒有人工指定人員類別）的向下相容備援邏輯。"""
    c = (code or '').strip().upper()
    return SALARY_TABLE_CATEGORY.get(c)


# 細分類選項：沿用 NEVER_EXCLUDE_TITLES（本檔下方，系統既有的正式職稱
# 清單）與既有排除規則裡已經在用的「約僱／約聘」「聘用人員」稱呼，
# 不是憑空新增的名詞。Leo 在網頁上為每個人指定的就是這裡的細分類。
FINE_CATEGORY_OPTIONS = [
    '校長', '主任', '組長', '教師', '導師', '科任', '教練',
    '幹事', '書記', '護理師', '校護', '管理員',
    '工友', '技工', '駕駛',
    '聘用人員',
    '約僱', '約聘',
...
FINE_TO_OFFICIAL = {
    '校長': '教育警察人員', '主任': '教育警察人員', '組長': '教育警察人員',
    '教師': '教育警察人員', '導師': '教育警察人員', '科任': '教育警察人員', '教練': '教育警察人員',
    '幹事': '一般人員', '書記': '一般人員', '護理師': '一般人員', '校護': '一般人員', '管理員': '一般人員',
    '工友': '職工', '技工': '職工', '駕駛': '職工',
    '聘用人員': '約聘僱人員',
    '約僱': '約聘僱人員', '約聘': '約聘僱人員',
}
...
def rollup_category(fine_category):
    """細分類 → 官方五類；辨識不出時回傳 None。"""
    return FINE_TO_OFFICIAL.get((fine_category or '').strip())
